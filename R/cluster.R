...
#' \describe{
#'   \item{\code{cluster_info}}{A data frame indicating the cluster number and distance to cluster center of
...
plot.hc_hurst<-function(x,type="estimates",...)
{
  ...
  if (type == "estimates")
  {
    ggplot(DF, aes(.data$t, .data$smth_est, group = .data$item)) +
  ...
  {
    ggplot(DF1,aes(.data$t,.data$values)) +
      facet_wrap(~clus, ncol = 2, scales = "fixed") +
      geom_line(color = "red") +
